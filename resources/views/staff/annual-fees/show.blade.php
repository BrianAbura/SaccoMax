                    ...
                    <div class="card">
                        <div class="card-header">
                            <h5 class="m-b-0 text-primary">
                                ...
                                <a href="{{ route('annual-fees.index') }}"><i
                        ...
                        <div class="card-body">
                            <div class="table-responsive custom-scrollbar">
                                <table class="display basic-1" id="basic-1">
                                    ...
                                    <tbody>
                                        @foreach ($annualfee as $annualfee)
                                            @php
                                                $cnt = $loop->iteration;
                                            @endphp
                                            <tr>
                                                <td>{{ $cnt }}</td>
                                                <td class="text-primary">UGX {{ number_format($annualfee->amount) }}</td>
                                                <td> {{ $annualfee->year }} </td>
                                                <td> {{ date('d-m-Y', strtotime($annualfee->payment_date)) }} </td>
                                                <td class="text-center">
                                                    @if (!empty($annualfee->receipt))
                                                        <a href="#" class="open-modal" data-bs-toggle="modal"
                                                            data-bs-target="#imageModal"
                                                            data-image="{{ asset($annualfee->receipt) }}">
                                                            <img class="b-r-10" src="{{ asset($annualfee->receipt) }}"
                                                                width="60px">
                                                        </a>
                                                    @else
                                                        -
                                                ...
                                                <td>
                                                    <ul class="action">
                                                        <li class="edit"> <a
                                                                href="{{ route('annual-fees.edit', $annualfee->id) }}"><i
